...
add 'on' function to fields and also allow change:fieldname from global
allow 'find' to dig to find by just name



change 'updates' to pub/sub model 1/3

fix form instance management - append form after removing old one
...
detect bool, radio, select as defaults, use format:'auto'

make 'inline' selection better, use renderer? no use bool, so it can be combined with wizard and tabs etc?
...
work on instance duplication and decoupling
*/

Berry = function(options, obj) {
//	temp.each(function(){console.log(this.getPath())})
			...
			if(this.options.flatten && true) {
				...
				this.each(function(){
					if(this.isContainer && this.owner.options.flatten && (this.instance_id === null || this.instance_id === 0 )){
						...
						//arr = _.difference(arr,this.owner.deflated);
						var baz = [];
				...
				//this.working =	this.deflate(this.working);
				// 			var n = {};
				// 			for(var i in this.working){
				// 				if(typeof this.working[i] === 'object'){
				// 					for(var j in this.working[i]){
				// 						n[i] = this.working[i][j];
				// 					}
				// 				}else{
				// 					n[i] = this.working[i];
				// 				}
	...
	this.populate = function(fields) {
		...
		this.each(function(){
			if(this.multiple){
				...
				var temp = {};
				// if(this.owner.options.flatten){
				// 	var arr = this.getPath().split('.');
				// 	var name = arr.pop();
				// 	temp = this.owner.toJSON(arr.join('.'));
				// 	temp[name] = this.owner.toJSON(this.name);
				// 	temp = temp[name];
				// }else{
	...
	var processMultiples = function(attributes) {
		...
		self.each(function(){
			if(this.multiple && this.toArray){
				var temp = this.owner.toJSON(this.getPath());
				//todo: make any depth
				//var root = this.owner.search(this.owner.altered, this.parent.getPath())

				var root = this.owner.attributes;
				if(this.isChild()){
					root = this.owner.search(this.owner.altered, this.parent.getPath());
				}
				//this.owner.altered[this.name] = {};
				root[this.name] = {};
				for(var i in this.children){
				//	this.owner.altered[this.name][i] = _.pluck(temp,i);

					//root[this.name][i] = _.pluck(temp,i);
	...
	//							this.owner.attributes[this.name].push(newObj);
	...
	var processMultiplesIN_old = function() {
		self.each(function() {
			if(this.multiple && this.toArray) {
...
//					var temp = this.owner.attributes[this.name].pop();
					...
					//for(var k in source) {
						for(var i in source[this.name]) {
							var newObj = $.extend({}, temp);
							for(var name in temp) {
								newObj[name] = source[this.name][i];
							}
//							this.owner.attributes[this.name].push(newObj);
							root[this.name].push(newObj);
						}
...
//				if(!field.isContainer) {
	...
	var parsefields = function(){
		self.each(function(){
			// var splits = this.getPath().split('.');
			// var target = this.owner.attributes;
			// for(var i in splits) {
			// 	if(typeof target[splits[i]] == 'object') {
			// 		target = target[splits[i]];
			// 	} else {
			// 		target[splits[i]] = this.getValue();
			// 	}
			// }
	...
	/*var runUpdate = function() {
		...
		do{
			...
			self.each(processShowItem);
			self.each(processDisabledItem);
			iteration++;
...
$.extend(Berry.field.prototype, {
	...
	getPath: function(){
		...
		if(this.parent !== null && this.parent !== undefined) {
			...
			// if(parent.multiple){
			// 	current.path += parent.instance_id + '.';
			// }
		}
	...
	getSearchPath: function(){
		...
		return path + this.name;// + '.' + this.instance_id;
	},
	isActive: function(){
...
//if bb Berry.field.extend = Backbone.Model.extend;
