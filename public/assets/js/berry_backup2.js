...
Berry = function(options, obj) {
	...
	this.toJSON = function(search) {
		if(typeof search === 'string'){
			...
			//possibly remove if I can rethink cleaning
	...
	this.on = function(topic, func, execute) {
		var eventSplitter = /\s+/;
	...
	this.processItem = function(item,target,source,parent,insert) {
		...
		if(Berry.types[item.type] !== undefined) {
			...
			if(typeof item.multiple !== 'undefined' && item.multiple && typeof source[(item.parseAs || item.name)] !== 'undefined'){
				...
				// if(item.isContainer && self.options.flatten) {
				// 	if(item.toArray){ cSource = normalizeFieldsetSource(source,'uuid'); }
				// 	//doesnt handle flatten
				// 	for(var f in source) {
				// 		initializeItem($.extend({},item),parent,target,source[f]);
				// 	}
				// } else {
...
Berry.field = function(item, owner){
    ...
    $.extend(this, this.item);
		...
		this.isActive = function(){
			return (this.removed !== true && (this.parent === null || this.parent === undefined || (this.parent.removed !== true && this.parent.enabled !== false)));
		}
...
$.extend(Berry.field.prototype, {
	...
	dupeMe: function() {
		...
		if(max == -1 || max > count){
			...
			this.owner.processItem(item, $(target), {}, this.parent, 'after').focus();
			this.trigger('change');
		}
	},
	dropMe: function() {
