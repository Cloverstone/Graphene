...
$('a[href],div[href]').live('click',function(event){
  if($.url($(this).attr('href')).attr('host')==""){
    event.preventDefault();
    event.stopPropagation()
    presend();
...
function hideMessage(){
  $('#status-message').fadeOut('slow').attr("class","alert alert-info");
};
...
$('#status-message').removeClass("alert-info");
  ...
  $("#wait").hide();$('#status-message').html(message).addClass("alert-"+status);$('#status-message').fadeIn('fast');clearTimeout(timer);timer=setTimeout(hideMessage,4000)
}
